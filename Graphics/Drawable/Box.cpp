	...
	phi(adist(rng))
{
	...
	AddBind(std::make_unique<VertexBuffer>(gfx, vertices));		// add vertex data
	...
	AddIndexBuffer(std::make_unique<IndexBuffer>(gfx, indices));	// add index buffer
	...
	AddBind(std::make_unique<InputLayout>(gfx, ied, pvsbc));	// bind input layout with vertex shader info
